...
#*******************************************************************************
"""Drive a CodeQL MISRA C++ analysis of Bazel-built C++ targets.

Ported from eclipse-score/communication's quality/static_analysis/codeql_lint.py
as part of the CodeQL evaluation for baselibs (eclipse-score/baselibs#317).

Two phases are supported, plus an "all" phase that runs both in sequence:
  - create-database:  init a CodeQL database and populate it by tracing a
                       `bazel build --config=codeql` of the given target(s).
  - analyze-database:  run the MISRA C++ coding-standards queries against an
                       existing database and emit SARIF + CSV reports.
"""
import argparse
import os
import tempfile
...
def create_database(code_ql_path, config_path, target, source_root, database_path):
    ...
    # Note: the `--` before database_path is required to mark the end of options so
    # CodeQL treats the path as the positional <database> argument, not a flag value.
    subprocess.run(
    ...
    subprocess.run(
        f"bazel run @codeql_coding_standards//:process_coding_standards_config -- "
        f"--working-dir={source_root}",
...
def analyze_database(code_ql_path, database_path, source_root, analysis_report_path=None,
                      query_spec=None, output_prefix="codeql", output_dir=None):
    ...
    if analysis_report_path and os.path.exists(analysis_report_path):
        ...
        try:
            ...
            # analysis_report's deviations/diagnostics/recategorizations reports run CodeQL
            # queries from the `codeql/common-cpp-coding-standards` qlpack (cpp/common/src),
            # which depends on `advanced-security/qtil`. Unlike a plain git clone of
            # codeql-coding-standards, our git_repository fetch does not pre-resolve qlpack
            # dependencies, so the first run fails with "Pack ... was not found in the pack
            # download cache" (eclipse-score/baselibs#317 finding). Install it explicitly.
            runfiles_dir = f"{analysis_report_path}.runfiles"
            for qlpack_dir in glob.glob(os.path.join(runfiles_dir, "*", "cpp", "common", "src")):
                subprocess.run([code_ql_path, "pack", "install", qlpack_dir], env=env, check=False)
            ...
            # NOTE: previously this only checked "any file exists", which silently
            # hid partial failures (e.g. only database_integrity_report.md written
            # while deviations_report.md etc. were missing due to a crash) - see
            # eclipse-score/baselibs#317 findings. Compare against the expected set.
            expected_report_files = {
                "database_integrity_report.md", "deviations_report.md",
                "guideline_compliance_summary.md", "guideline_recategorizations_report.md",
            }
            if os.path.exists(reports_output_dir):
                report_files = glob.glob(os.path.join(reports_output_dir, "*"))
                found_names = {os.path.basename(f) for f in report_files}
                missing = expected_report_files - found_names
                if report_files:
                    print(f"Reports generated successfully" if not missing
                          else f"Reports partially generated (missing: {', '.join(sorted(missing))})")
                    print(f"\n  Generated Report Files:")
                    for f in sorted(report_files):
                        file_size = os.path.getsize(f) / 1024  # KB
                        print(f"    {os.path.basename(f)} ({file_size:.1f} KB)")
                    if missing:
                        print(f"  analysis_report stderr: {result.stderr}")
                else:
            ...
            print(f"Report generation warning: {e.stderr if e.stderr else e}")
    else:
