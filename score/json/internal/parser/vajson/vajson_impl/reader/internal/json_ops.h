...
class JsonOps final
{
  public:
    ...
    /// \threadsafe      FALSE
    /// \reentrant       FALSE
    auto CheckString(std::string_view string, std::string_view error_msg) noexcept -> Result<Blank>;
  ...
  private:
    ...
    /// \threadsafe      FALSE
    /// \reentrant       FALSE
    auto RewindIf(bool condition, std::size_t num) noexcept -> Result<Blank>;
