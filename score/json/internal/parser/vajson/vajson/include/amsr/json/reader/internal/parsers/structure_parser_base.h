...
class StructureParserBase {
 public:
   ...
   */
  StructureParserBase(StructureParserBase const&) = delete;
  /*!
  ...
  StructureParserBase& operator=(StructureParserBase const&) = delete;
   ...
   * \endinternal
   */
  virtual auto OnBinary(score::cpp::span<char const>) noexcept -> ParserResult = 0;
   ...
   * \endspec
   */
  virtual auto GetJsonOps() const& noexcept -> JsonOps const& = 0;
   ...
   * \endspec
   */
  auto ReadBinary(score::cpp::move_only_function<ParserResult(StringView)> const& callback) noexcept -> ParserResult;
