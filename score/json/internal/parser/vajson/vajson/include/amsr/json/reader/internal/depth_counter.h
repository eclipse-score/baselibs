...
class DepthCounter final {
 ...
 public:
   ...
   * \details         An EOF is correct when there are no more elements on the stack, i.e. no arrays, objects, or keys
  ...
  auto CheckEndOfFile() const noexcept -> ParserResult {
    ...
    if (!is_finished_ && IsEmpty()) {
      result = MakeErrorResult<ParserState>(amsr::json::JsonErrc::kInvalidJson, "DepthCounter::CheckEndOfFile: Empty document.");
    } else if (!is_finished_ && !IsEmpty() && stack_.top() == '[') {
      result = MakeErrorResult<ParserState>(amsr::json::JsonErrc::kInvalidJson, "DepthCounter::CheckEndOfFile: Expected closing brackets.");
    } else if (!is_finished_ && !IsEmpty() && stack_.top() != '[') {
      result = MakeErrorResult<ParserState>(amsr::json::JsonErrc::kInvalidJson, "DepthCounter::CheckEndOfFile: Expected closing braces.");
    } else {
      // already assigned
    }
    return result;
   ...
   * \endspec
   */
  auto AddArray() noexcept -> ResultBlank { return this->AddElement('['); }
   ...
   * \endspec
   */
  auto AddObject() noexcept -> ResultBlank { return this->AddElement('{'); }
  ...
  auto PopObject() noexcept -> Result<std::size_t> {
    ...
    return MakeResult(expected_key, MakeError(static_cast<amsr::json::ErrorCode>(JsonErrc::kInvalidJson), "DepthCounter::PopObject: Not in an object."))
  ...
  auto PopArray() noexcept -> Result<std::size_t> {
    bool const expected_key{this->CheckLastElement('[')};
    return MakeResult(expected_key, MakeError(static_cast<ErrorCode>(JsonErrc::kInvalidJson), "DepthCounter::PopArray: Not in an array."))
        .transform([this](score::Blank) noexcept {
          this->comma_expected_ = true;
          return this->Pop();
        });
