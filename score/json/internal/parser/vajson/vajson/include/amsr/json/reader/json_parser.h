...
class JsonParser final {
 public:
   ...
   * \error           amsr::json::JsonErrc::kUserValidationFailed
   *                  if checking for a specific name of a key or string but the next key or string has a different name
   * \context         ANY
   * \pre             -
   * \threadsafe      FALSE
   * \reentrant       FALSE
   ...
   * \endinternal
   */
  auto Validate() const noexcept -> ResultBlank { return this->result_; }
   ...
   * \endspec
   */
  auto GetJsonDocument() noexcept -> JsonData& { return this->data_.get(); }
   ...
   * \endspec
   */
  auto GetJsonDocument() const noexcept -> JsonData const& { return this->data_.get(); }
   ...
   *                  ResultBlank. The provided StringView is only valid until any other method or parser operating on
  ...
  auto Key(std::string_view key) noexcept -> JsonParser& {
    return this->IfValid([this, &key]() noexcept { return this->parser_.Key(key); });
  }
   ...
   * \endspec
   */
  auto Key(std::string const& key) noexcept -> JsonParser& { return this->Key(std::string_view{key}); }
  ...
  auto Bool(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.Bool(fn).Drop(); });
  }
   ...
   *                  provided StringView is only valid until any other method or parser operating on the same document
   *                  is called.
   * \vpublic
   * \tparam          Fn
   *                  Type of callable.
  ...
  auto String(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.String(fn).Drop(); });
  }
  ...
  auto String(std::string_view string) noexcept -> JsonParser& {
    return this->IfValid([this, &string]() noexcept { return Drop(this->parser_.String(string)); });
  }
   ...
   * \endspec
   */
  // auto String(CStringView string) noexcept -> JsonParser& { return this->String(std::string_view(string.c_str(), string.size())); }
   ...
   * \endspec
   */
  auto String(std::string const& string) noexcept -> JsonParser& {
  ...
  auto Number(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.Number<Num>(fn).Drop(); });
  }

  /*!
   * \brief           Checks if the next token is binary content and executes the given callable
   * \details         The callable must take the binary content as score::cpp::span<char const> and return ResultBlank. The
  ...
  auto Binary(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.Binary(fn).Drop(); });
  }
   ...
   *                  successful Result or has return type void. If the callable is unable to consume all tokens it must
  ...
  auto Array(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.Array(fn).Drop(); });
  }
  ...
  auto StringArray(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.StringArray(fn).Drop(); });
  }

  /*!
   * \brief           Checks if the next token is an array of numbers and executes the given callable on every element
   * \details         The callable must take the current array index as a std::size_t, the current number of template
  ...
  auto NumberArray(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.NumberArray<Num>(fn).Drop(); });
  }
  ...
  auto BoolArray(Fn const& fn) noexcept -> JsonParser& {
    return this->IfValid([this, &fn]() noexcept { return this->parser_.BoolArray(fn).Drop(); });
  }
   ...
   *                  ResultBlank. The provided StringView is only valid until any other method or parser operating on
   *                  the same document is called. The callable is expected to have consumed all tokens representing the
   *                  value related to the key if it returns a successful Result or has return type void. If the
   *                  callable is unable to consume all tokens it must return an error Result.
   * \vpublic
   * \tparam          Fn
   *                  Type of callable.
   * \param[in]       fn
  ...
  auto Object(Fn const& fn, bool object_already_open = false) noexcept -> JsonParser& {
    return this->IfValid(
        [this, &fn, object_already_open]() noexcept { return this->parser_.Object(fn, object_already_open).Drop(); });
  }
   ...
   * \endinternal
   */
  auto AddErrorInfo(char const* msg) & noexcept -> JsonParser& {
 ...
 private:
  ...
  auto IfValid(Fn const& fn) & noexcept -> JsonParser& {
    if (this->result_) {
      ...
      // call is still just a positive ResultBlank. Thus, even if the parser call returned successfully, this->result_
