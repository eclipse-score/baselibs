...
class NumberParser {
 public:
  ...
  auto End() const&& noexcept -> char const* = delete;
...
class JsonNumber final {
 public:
  ...
  static auto New(StringView view) noexcept -> Result<JsonNumber> {
    return Filter(Filter(Result<JsonNumber>{JsonNumber{view}}, [](JsonNumber const& num) noexcept { return !num.view_.empty(); },   MakeError(static_cast<ErrorCode>(JsonErrc::kInvalidJson), "JsonNumber::New: View must not be empty."))
        ,[](JsonNumber const& num) noexcept { return num.Validate(); },
                MakeError(static_cast<ErrorCode>(JsonErrc::kInvalidJson), "JsonNumber::New: Number contains invalid characters."));
  };
