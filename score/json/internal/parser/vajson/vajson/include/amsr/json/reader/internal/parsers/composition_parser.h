...
using ReturnsResult = score::IsResult<std::result_of_t<F(T const&)>>;
 ...
 */
template <typename F, typename T>
using ReturnsResultVoid = std::is_same<ResultBlank, std::result_of_t<F(T const&)>>;
 ...
 */
template <typename F, typename T>
using ArrayReturnsResult = score::IsResult<std::result_of_t<F(std::size_t, T const&)>>;
 ...
 */
template <typename F, typename T>
using ArrayReturnsResultVoid = std::is_same<Result<score::Blank>, std::result_of_t<F(std::size_t, T const&)>>;
...
class CompositionParser : public Mixin {
 ...
 public:
  ...
  CompositionParser& operator=(CompositionParser const&) = delete;
   ...
   * \details         The callable must take the current array index as a std::size_t, the current number of type T, and
   *                  return void.
   * \vpublic
   *
   * \tparam          T
  ...
  /*!
   * \brief           Parses the following binary content and executes the given callable
   * \details         The callable must take the binary content as score::cpp::span<char const> and return void. The provided
   ...
   * \details         The callable must take the current array index as a std::size_t and return void. The callable must
   ...
   *                  successful Result. If the callable is unable to consume all tokens it must return an error Result.
   * \vpublic
   *
   * \tparam          Fn
   *                  Type of callable.
   ...
   *                  provided StringView is only valid until any other method or parser operating on the same document
   *                  is called. The callable must consume all tokens representing the value related to the key.
   * \vpublic
   *
   * \tparam          Fn
   *                  Type of callable.
  ...
  auto Object(Fn fn, bool object_already_open = false) noexcept
      -> CallableReturnsNoResult<Fn, std::string_view, R> {
    static_assert(ReturnsVoid<Fn, std::string_view>::value, "Must return void");
    return Object(
   ...
   *                  provided StringView is only valid until any other method or parser operating on the same document
   *                  is called. The callable is expected to have consumed all tokens representing the value related to
   *                  the key if it returns a successful Result. If the callable is unable to consume all tokens it must
