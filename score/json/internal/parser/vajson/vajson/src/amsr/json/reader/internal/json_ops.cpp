...
auto JsonOps::Take() noexcept -> char { return this->TryTake().value(); }
...
auto JsonOps::Tell() const noexcept -> Result<std::uint64_t> {
  ...
  if (pos == std::streampos(-1)) {
    result = MakeErrorResult<std::uint64_t>(JsonErrc::kStreamFailure, "JsonOps::Tell: Could not get stream position.");
  }
 ...
 * \endinternal
 */
auto JsonOps::Skip(char const character) noexcept -> bool {
  std::istream& stream = this->GetStream();
  std::int64_t peek_val = stream.peek();
  bool result{false};
  if (peek_val != std::char_traits<char>::eof() &&
      peek_val == std::char_traits<char>::to_int_type(character)) {
    stream.get();
    result = !stream.fail();
  }
...
auto JsonOps::CheckString(std::string_view const string, std::string_view const error_msg) noexcept
  ...
  return ReadString(string)
      .and_then([&error_msg](bool val) noexcept {
...
auto JsonOps::ReadString(std::string_view const string) noexcept -> Result<bool> {
  ...
  return this
      ...
      .transform([&is_same](score::Blank) noexcept { return is_same; });
}
 ...
 * \endinternal
 */
auto JsonOps::RewindIf(bool const condition, std::size_t const num) noexcept -> ResultBlank {
  ResultBlank result{};
  if (condition) {
    std::istream& stream = this->GetStream();
...
auto JsonOps::SkipWhitespace() noexcept -> bool {
  ...
  while (!done) {
    ...
    if (ch == std::char_traits<char>::eof()) {
      ...
      bool const is_whitespace = std::any_of(kWhitespaceChars.cbegin(), kWhitespaceChars.cend(),
                                        [c](char wc) noexcept { return c == wc; });
...
auto JsonOps::Read(std::uint64_t const num_to_read,
                   score::cpp::move_only_function<void(std::string_view), 40> const& callback) noexcept
    -> Result<std::uint64_t> {
  std::istream& stream = this->GetStream();
...
auto JsonOps::ReadExactly(std::uint64_t const num_to_read,
  ...
  Result<score::Blank> result = Drop(Read(num_to_read,
                [&buffer, &callback, &num_to_read, &callback_executed](std::string_view view) noexcept {
               ...
               and_then([&num_to_read](std::uint64_t const& read) noexcept {
                 score::ResultBlank result{MakeErrorResult<score::Blank>(JsonErrc::kInvalidJson, "JsonOps::ReadExactly: Unexpected EOF.")};
                 if (num_to_read == read) {
                    result.emplace(score::Blank{});
                 }
                return result; }));
...
auto JsonOps::ReadUntil(std::string_view const delimiter,
  ...
  while (!done) {
    ...
    if (ch == std::char_traits<char>::eof()) {
      ...
      char const character{std::char_traits<char>::to_char_type(static_cast<std::int32_t>(ch))};

      // Check if this character matches any in the delimiter
      bool const is_delimiter = std::any_of(delimiter.cbegin(), delimiter.cend(),
                                        [character](char delim) noexcept { return character == delim; });
