...
class DefaultKeySerializer
{
  public:
    ...
    /// \param[in] value to serialize as a key. Must be convertible to a StringView type.
    /// \return The serializable key type.
    template <typename T>
    auto operator()(const T& value) const noexcept -> JKeyType
    {
...
class PairRangeSerializer final
{
  public:
    ...
    /// \param[in] key_fn Function used to serialize single keys.
    /// \param[in] value_fn Function used to serialize single values.
    /// \pre The passed functions & range do not throw any exceptions
    template <typename KeyFn1 = KeyFn, typename ValueFn1 = ValueFn>
    PairRangeSerializer(const Range& range, KeyFn1&& key_fn, ValueFn1&& value_fn) noexcept
    ...
    /// \param[in] os Object serializer to write into.
  ...
  private:
    ...
    std::reference_wrapper<const Range> map_;
...
auto JObject(const Range& range,
             KeyFn&& key_fn = DefaultKeySerializer{},
             ValueFn&& value_fn = IdSerializer<Range>{}) noexcept
    -> JObjectType<PairRangeSerializer<Range, KeyFn, ValueFn>>
{
    return {PairRangeSerializer<Range, KeyFn, ValueFn>{
        range, std::forward<KeyFn>(key_fn), std::forward<ValueFn>(value_fn)}};
}
