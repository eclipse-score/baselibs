...
class Signal
{
  public:
    ...
    /**
     * A note on the Sighandler* signal(const std::int32_t sig, Sighandler handler)
     * We choose not to support this function as its behavior varies across UNIX versions and implementations.
     * Please use SigAction instead.
     */
    ...
    virtual score::cpp::expected<std::int32_t, Error> SigWait(const sigset_t& set,
                                                              std::int32_t& sig) const noexcept = 0;

    virtual score::cpp::expected<std::int32_t, Error> SigAddSet(sigset_t& set,
                                                                const std::int32_t signo) const noexcept = 0;
