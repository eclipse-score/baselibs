...
score::cpp::optional<std::uint32_t> NetutilsImpl::get_default_gateway_ip4() const noexcept
{
    ...
    cur_rt.header.rtm_msglen = sizeof(struct rt_msghdr) + sizeof(struct sockaddr_in);
    cur_rt.header.rtm_version = RTM_VERSION;
    cur_rt.header.rtm_type = RTM_GET;
    cur_rt.header.rtm_flags = RTF_UP;
    cur_rt.header.rtm_addrs = RTA_DST;
    cur_rt.header.rtm_pid = pid;
    cur_rt.header.rtm_seq = SEQ;

    cur_rt.dst.sin_len = sizeof(struct sockaddr_in);
    cur_rt.dst.sin_family = AF_INET;
    cur_rt.dst.sin_addr.s_addr = INADDR_ANY;
    ...
    if (!(cur_rt.header.rtm_flags & RTF_GATEWAY))
    {
        score::mw::log::LogError(kLogContext) << "Default gateway address is not set.";
        return gateway_addr;
    }
