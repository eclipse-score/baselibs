...
- SERIALIZE-LAYOUT: The size and content of `::score::common::visitor::serialized_t<A, T>` shall be defined as follows:
  ...
  - for C-style arrays, `std::array<>`, `std::tuple<>`, `std::pair<>`, structs (with the help of `SCORE_STRUCT_VISITABLE()` macro) the serialized object consists of the concatenated (without padding) serialized objects of their subjects traversed in their natural order (for structs, the "natural" order is the order defined in their corresponding `SCORE_STRUCT_VISITABLE()` macro).
