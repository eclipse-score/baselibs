...
//! Lifetime-safe Rust bindings for C++ `score::cpp::stop_token` and
...
mod ffi {
    unsafe extern "C++" {
        ...
        fn stop_source_get_token(source: &stop_source) -> SharedPtr<stop_token>;
        ...
        #[namespace = "score::language::rust::stop_token"]
        #[cxx_name = "MakeDefaultStopToken"]
        fn make_default_stop_token() -> SharedPtr<stop_token>;

        #[namespace = "score::language::rust::stop_token"]
        #[cxx_name = "MakeStopSource"]
        fn make_stop_source() -> SharedPtr<stop_source>;
    }

    // This forces CXX to generate glue code for the shared pointers.
    impl SharedPtr<stop_token> {}
    impl SharedPtr<stop_source> {}
}
...
pub fn make_default_stop_token() -> cxx::SharedPtr<StopToken> {
    ffi::make_default_stop_token()
}

/// Creates an owned source with a new associated stop state.
pub fn make_stop_source() -> cxx::SharedPtr<StopSource> {
...
impl StopSource {
    ...
    pub fn get_token(&self) -> cxx::SharedPtr<StopToken> {
...
#[cfg(test)]
mod tests {
    use super::{make_default_stop_token, make_stop_source};

    #[test]
    fn default_token_has_no_stop_state() {
        let token = make_default_stop_token();
