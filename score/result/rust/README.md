...
- **`score::Result<T>`** - C++ result type with error handling
- **`score::result::Error`** - Error objects with domain and error codes
...
- **C++ Side**: Creation of `score::Result<T>` with discriminant and storage union
...
- [score::Result Documentation](../result.h) - C++ Result type
