...
.. document:: log_frontend Requirements Inspection Checklist
  :id: doc__log_req_inspection
  :status: draft
  :version: 1
  :safety: ASIL_B
  :security: YES
...
.. needtable::
   :filter: docname is not None and "mw_log" in docname and "requirements" in docname and status == "valid"
   :style: table
   :types: comp_req
...
.. needtable::
   :filter: docname is not None and "mw_log" in docname and "safety_analysis" in docname and status == "valid"
   :style: table
   :types: aou_req
   :columns: id;status;tags
   :colwidths: 25,25,25
